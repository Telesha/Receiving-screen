...
export default function AccountingSuppliersListing(props) {
  ...
  const handleClick = () => {
    encryptedID = btoa('0');
    navigate('/app/accountingSuppliers/addedit/' + encryptedID);
  }
  ...
  async function getPermission() {
    ...
    var isAuthorized = permissions.find(p => p.permissionCode == 'VIEWLISTINGACCOUNTINGSUPPLIERS');
  ...
  async function getSuppliersByGroupIDAndFactoryID() {
    var result = await services.getSuppliersByGroupIDAndFactoryID(parseInt(supplierList.groupID));
      ...
      <Grid container spacing={1}>
        ...
        <Grid item md={2} xs={12}>
          <PageHeader
            onClick={handleClick}
            isEdit={true}
            toolTiptitle={"Add Accounting Suppliers"}
          />
        </Grid>
      </Grid>
    )
    ...
    >
      <Container maxWidth={false}>
        <Box mt={0}>
          <Card>
            <CardHeader
              title={cardTitle("Accounting Suppliers")}
            />
            <PerfectScrollbar>
              <Divider />
