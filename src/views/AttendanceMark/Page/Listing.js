...
export default function AttendanceMarkListing(probs) {
    ...
    useEffect(() => {
        const IDdata = JSON.parse(
            sessionStorage.getItem('attendanceMark-listing-page-search-parameters-id')
        );
        trackPromise(
    ...
    useEffect(() => {
        sessionStorage.removeItem(
            'attendanceMark-listing-page-search-parameters-id'
        );
    }, []);
    ...
    async function getPermissions(IDdata) {
        ...
        var isAuthorized = permissions.find(p => p.permissionCode == 'VIEWATTENDANCE');
        ...
        if (isInitialLoad) {
            ...
            GetPreviousData(IDdata);
        }
    }

    async function GetPreviousData(IDdata) {
        let model = {
            groupID: parseInt(IDdata.groupID),
            estateID: parseInt(IDdata.estateID),
            regNo: IDdata.regNo,
            designationID: parseInt(IDdata.designationID),
            month: parseInt(IDdata.month),
            year: parseInt(IDdata.year)
        }
    ...
    async function getDesignationsForDropdown() {
        const designation = await services.getDesignationsByFactoryID(attenceFormData.estateID);
    ...
    const handleClick = () => {
        encrypted = btoa('0');
        navigate('/app/attendanceMark/addEdit/' + encrypted);
    }
            ...
            <Grid container spacing={1}>
                ...
                <Grid item md={2} xs={12}>
                    <PageHeader
                        onClick={handleClick}
                        isEdit={true}
                        toolTiptitle={"Add Attendance Mark"}
                    />
                </Grid>
            </Grid>
        )
    ...
    const handleClickEdit = (attendanceMarkID) => {
        encrypted = btoa(attendanceMarkID.toString());
        let modelID = {
            groupID: parseInt(attenceFormData.groupID),
            estateID: parseInt(attenceFormData.estateID),
            regNo: employeeNumber ? employeeNumber : "",
        ...
        sessionStorage.setItem('attendanceMark-listing-page-search-parameters-id',
    ...
    const deleteDetails = async () => {
        if (deleteIndex !== null) {
            let model = {
                attendanceMarkID: deleteIndex,
                modifiedBy: parseInt(tokenService.getUserIDFromToken())
            }
            const res = await services.DeleteAttendanceMark(model);
                                            ...
                                            <Box minWidth={1050}>
                                                {attendanceData.length > 0 && isShowTable ? (
                                                    <MaterialTable
                                                        ...
                                                        columns={[
                                                            { title: 'Reg Number', field: 'regNo' },
                                                            { title: 'Employee Name', field: 'employeeName' },
                                                            { title: 'Designation', field: 'designation' },
                                                            { title: 'Attendance Count', field: 'attendanceCount' },
                                                        ...
                                                        actions={[
                                                            {
                                                                ...
                                                                onClick: (event, rowData) => { handleClickEdit(rowData.attendanceMarkID) }
                                                            },
                                                            {
                                                                icon: 'delete',
                                                                tooltip: 'Delete',
                                                                onClick: (event, rowData) => showDeleteConfirmation(rowData.attendanceMarkID)
                                                            }
                                                        ]}
                                                    />
                                                ) : null}
