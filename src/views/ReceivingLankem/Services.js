...
export default {
  ...
  getPayVoucherCode,
  getCustomersPaymentID,
  getLedgerAccountTRCode,
  getCustomersForDropDownAuto,
  getTranLedgerListForDropdownAuto
};
...
async function saveGeneralJournal(generalJournal) {
  let saveModel = {
    ...
    customerID: parseInt(generalJournal.customerID),
...
async function getTranLedgerListForDropdownAuto(factoryID) {
  const response = await CommonGet('/api/GeneralJournal/GetTranLedgerListForDropdown', "factoryID=" + parseInt(factoryID));
  return response.data;
}

async function getCustomersForDropDown(factoryID) {
  var customerArray = [];

  const response = await CommonGet('/api/AccountingCustomers/GetCustomersForDropDown', "factoryID=" + parseInt(factoryID));
  for (let item of Object.entries(response.data)) {
...
async function getCustomersForDropDownAuto(factoryID) {
  const response = await CommonGet('/api/AccountingCustomers/GetCustomersForDropDown', "factoryID=" + parseInt(factoryID));
  return response.data;
}

async function getPayVoucherCode() {
  const response = await CommonGet('/api/VoucherType/GetReceiveVoucherCode', null)
  return response.data;
}

async function getCustomersPaymentID(factoryID, customerID) {
  var customerArray = [];

  const response = await CommonGet('/api/AccountingCustomers/GetCustomersReceiptData', "factoryID=" + parseInt(factoryID) + "&customerID" + parseInt(customerID));
  return response.data;
}
