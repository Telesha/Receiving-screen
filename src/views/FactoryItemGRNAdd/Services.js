...
async function getFactoryGRNByFactoryItemID(factoryID, factoryItemID, itemCategoryID) {
  const response = await CommonGet('/api/FactoryItemGrnAdd/GetFactoryGRNAddByFactoryItemIDandFactoryID', "factoryID=" + parseInt(factoryID) + '&factoryItemID=' + parseInt(factoryItemID) + '&itemCategoryID=' + parseInt(itemCategoryID));
...
async function saveFactoryGrnAdd(factoryGrn, grnList, dueDate, invoiceDate, receiveDate, factoryGRN, tempSuplierID) {
  let saveModel = {
    factoryItemGRNID: 0,
    ...
    locationID: parseFloat(factoryGRN.locationID),
  ...
  const response = await CommonPost('/api/FactoryItemGrnAdd/SaveFactoryItemGRNAddV2', null, saveModel);
  return response;
}
...
async function updateFactoryItem(factoryItemGRNID, factoryGRN, grnlist, dueDate, invoiceDate, receiveDate) {
  ...
  const response = await CommonPost('/api/FactoryItemGrnAdd/UpdateFactoryItemGRNAddV2', null, updateModel);
  return response;

}

async function getGRNDetailsByID(factoryItemGRNAddID) {
  const response = await CommonGet('/api/FactoryItemGrnAdd/GetGRNAddDetailsByID', "factoryItemGRNAddID=" + parseInt(factoryItemGRNAddID));
  return response.data;
}
...
async function getPurchaseOrderListing(factoryID) {
  const response = await CommonGet('/api/FactoryItemGrnAdd/GetPurchaseOrderDetailsByFactoryIDForGrnAdd', 'factoryID=' + parseInt(factoryID))
...
async function getSupplierByPurchaseOrderID(purchaseOrderID) {
  const response = await CommonGet('/api/PurchaseOrder/GetSupplierByPurchaseOrderIDForGRNAdd', 'purchaseOrderID=' + purchaseOrderID)
  return response.data;
}
