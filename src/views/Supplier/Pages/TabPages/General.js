        ...
        <Formik
          ...
          validationSchema={
            Yup.object().shape({
              ...
              address1: Yup.string().max(255).matches(/^[a-zA-Z0-9\d\s\,\.\/]+$/, 'Special characters are not allowed').required('Address 1 is required').max('20', 'Allow only 20 digits'),
              address2: Yup.string().max(255).matches(/^[a-zA-Z0-9\d\s\,\.\/]+$/, 'Special characters are not allowed').required('Address 2 is required').max('20', 'Allow only 20 digits'),
              ...
              officePhoneNumber: Yup.string().matches(/^((\\+[1-9]{1,4}[ \\-]*)|(\\([0-9]{2,3}\\)[ \\-]*)|([0-9]{2,4})[ \\-]*)*?[0-9]{3,4}?[ \\-]*[0-9]{3,4}?$/, 'Please enter valid phone number').
                min(10, "Enter number is too short").max(10, "Entered number is too long").required('Office phone number is required'),
              ...
              supplierName: Yup.string().required('Supplier Name is required').matches(/^[a-zA-Z0-9_ ]*$/, 'Special characters are not allowed').max('50', 'Allow only 50 digits'),
            })
          }
