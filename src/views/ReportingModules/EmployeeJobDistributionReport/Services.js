...
async function getAttendanceDetailsForReport(model) {
    let response = await CommonPost('/api/EmployeeWiseJobDistribution/GetEmployeeWiseJobDistributionDetails', null, model);
    return response;
}

async function GetPluckingJobTypesByJobTypeID(JobTypeID) {
  let response = await CommonGet('/api/EmployeeWiseJobDistribution/GetPluckingJobTypesByJobType',"JobTypeID=" +parseInt(JobTypeID));
