...
const screenCode = 'BALANCEPAYMENTCHECKLISTREPORT';

export default function BalancePaymentCheckListReport() {
  const [title, setTitle] = useState('Balance Payment CheckList Report');
  ...
  async function getPermission() {
    ...
    var isAuthorized = permissions.find(
      p => p.permissionCode == 'VIEWBALANCEPAYMENTCHECKLISTREPORT'
    );
  ...
  async function createDataForExcel(array) {
    ...
    if (array != null) {
      ...
      var vr1 = {
        'Supplier Name': "Group :" + selectedSearchValues.groupName,
        'Registration Number': "Factory :" + selectedSearchValues.factoryName
      };
      res.push(vr1);
      var vr2 = {
        'Supplier Name': "Route :" + selectedSearchValues.routeName,
        'Registration Number': "Month : " + selectedSearchValues.monthName + "," + selectedSearchValues.year
      };
      res.push(vr2);
    }
  ...
  async function createFile() {
    ...
    var settings = {
      sheetName: 'Balance Payment Checklist Report',
      fileName:
        'Balance Payment Checklist Report ' +
        selectedSearchValues.factoryName +
        ' - ' +
        selectedSearchValues.groupName +
        ' BALANCE PAYMENT CHECKLIST REPORT FOR THE MONTH OF ' +
        selectedSearchValues.monthName +
        ' - ' +
        balancePaymentListData[0].applicableYear,
      writeOptions: {}
    };
    ...
    let dataA = [
      {
        sheet: 'Balance Payment List Report',
                    ...
                    <PerfectScrollbar>
                      ...
                      {balancePaymentListData.length > 0 ? (
                        <Box display="flex" justifyContent="flex-end" p={2}>
                          ...
                          <ReactToPrint
                            documentTitle={'Balance Payment CheckList Report'}
                            trigger={() => (
                              <Button
                                color="primary"
                                id="btnRecord"
                          ...
                          <div hidden={true}>
                            <CreatePDF
                              ...
                              balancePaymentListData={balancePaymentListData}
                              total={totalAmount}
                            />
                          </div>
                        </Box>
