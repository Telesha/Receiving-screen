...
export default {
  ...
  async function GetLedgerAccountNamesandCodeForDropdown(groupID, factoryID) {
    ...
    const response = await CommonGet('/api/DailyPaymentReport/GetLedgerAccountNamesandCodeForDropdown',
      "groupID=" + parseInt(groupID) + "&factoryID=" + parseInt(factoryID));
    return response.data;
  } 

  async function getLedgerTrasactionDetails(dailyPaymentReportModel) {
    let response = await CommonPost('/api/DailyPaymentReport/GenerateLedgerAccountDailyPaymentReport', null, dailyPaymentReportModel);
    return response;
  }
