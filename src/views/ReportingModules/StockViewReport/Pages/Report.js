...
const screenCode = 'STOCKVIEWREPORT';

export default function StockViewReport(props) {
    const [title, setTitle] = useState("Stock View Report")
    ...
    async function getPermission() {
        ...
        var isAuthorized = permissions.find(p => p.permissionCode == 'VIEWSTOCKVIEWREPORT');
        ...
        if (array != null) {
            array.map(x => {
                var vr = {
                    ...
                    'Item': x.itemName,
                    'Available Quantity': x.availableQuantity,
                }
                res.push(vr);
            });
        }
    ...
    async function createFile() {
        ...
        var settings = {
            sheetName: 'Stock View Report',
            fileName: 'Stock View Report  ' + ' - ' + selectedSearchValues.groupName + ' - ' + selectedSearchValues.factoryName + ' - ' + stockBalanceReportInput.date,
            writeOptions: {}
        }
