...
export function Payments({ customerPaymentArray, setCustomerPaymentArray }) {
  ...
  async function setPaymentValues() {
    if (Object.keys(customerPaymentArray).length > 0) {
      setCustomerPaymentMethodID(customerPaymentArray[0].customerPaymentMethodID);
    }
  }
  ...
  async function savePaymentMethods(values) {
    ...
    setIsEdit(false);
    var paymentDetails = customerPaymentArray;

    let items = customerPaymentArray.filter((x => x.paymentTypeID == parseInt(values.paymentTypeID)));
    ...
    if (duplicate.length > 0) {
      ...
      setCustomerPaymentArray(paymentDetails);
  ...
  async function confirmData() {
    ...
    if (customerPaymentArray[tableIndex].factoryItemCustomerPaymentID != undefined || customerPaymentArray[tableIndex].factoryItemCustomerPaymentID != null) {
      const res = await services.DeActivateCustomerPaymentMethod(customerPaymentArray[tableIndex].factoryItemCustomerPaymentID);
      setCustomerPaymentArray(current => current.filter((img, i) => i != tableIndex));
      if (res == 1) {
      ...
      for (var i = 0; i < customerPaymentArray.length; i++) {
        setPayment({
          ...payment,
          paymentTypeID: customerPaymentArray[i].paymentTypeID,
          bankID: customerPaymentArray[i].bankID,
          branchID: customerPaymentArray[i].branchID,
          accountNumber: customerPaymentArray[i].accountNumber,
          accountHolderName: customerPaymentArray[i].accountHolderName
        })
      }
      customerPaymentArray.splice(tableIndex, 1);
      alert.success('PAYMENT TYPE DELETED SUCCESSFULLY');
      setDialogbox(false);
      setPayment({
        ...payment, paymentTypeID: '0', bankID: '0', branchID: '0', accountNumber: '', accountHolderName: ''
  ...
  return (
    <Fragment>
      <LoadingComponent />
      <Page className={classes.root} title="Factory Item Customer Payments Add Edit">
                    ...
                    <PerfectScrollbar>
                        ...
                        <Grid container spacing={3}>
                          <Grid item md={4} xs={12}>
                            ...
                            <TextField select fullWidth
                              ...
                              variant="outlined"
                              size='small'
                              disabled={(customerPaymentArray.length <= 0) ? false : true}
                          ...
                          {payment.paymentTypeID === "1" || payment.paymentTypeID === "2" ?
                            <Grid item md={4} xs={12}>
                      ...
                      <Box display="flex" justifyContent="flex-end" p={2}>
                        <Button
                          ...
                          type="submit"
                          variant="contained"
                          disabled={isSubmitting || (customerPaymentArray.length <= 0) ? false : true}
                        >
                          Add
                        </Button>
                      </Box>
                            ...
                            <Box minWidth={1050}>
                              <TableContainer >
                                <Table className={classes.table} aria-label="caption table">
                                  ...
                                  <TableBody>
                                    {customerPaymentArray.map((rowData, index) => (
                                      <TableRow key={index}>
