...
import {
  ...
  TableCell,
  Table,
  TableBody, Dialog, DialogActions, DialogContent, DialogTitle, Slide
...
export default function MusterChitAddEdit(props) {
  ...
  const [lentsectionTypes, setLentSectionTypes] = useState([]);
  ...
  useEffect(() => {
    getFieldDetailsByLentSectionTypeID()
  }, [FormDetails.lentSectionTypeID, FormDetails.lentdivisionID])
  ...
  async function getFieldDetailsByLentSectionTypeID() {
    var response = await services.getFieldDetailsByDivisionIDSectionTypeID(FormDetails.lentSectionTypeID, FormDetails.lentdivisionID);
  ...
  async function GetSundryJobTypesByJobCategoryID() {
    var response = await services.GetSundryJobTypesByJobCategoryID(FormDetails.estateID);
    setSundryJobType(response);
  };

  async function getPermissions() {
    var permissions = await authService.getPermissionsByScreen(screenCode);
    var isAuthorized = permissions.find(p => p.permissionCode == 'ADDEDITMUSTERCHITNEW');
    ...
    return (
      <Grid container spacing={1}>
        ...
        <Grid item md={2} xs={12}>
          <PageHeader
            onClick={handleClick}
            toolTiptitle={"Back to Muster Chit View"}
          />
        </Grid>
      </Grid>
    )
    ...
    if (radioValue2 == true || radioValue3 == true) {
      ...
      if (isJobTypeAlreadyAdded) {
        ...
        array1.push({
          ...
          lentSectionTypeName: lentsectionTypes[FormDetails.lentSectionTypeID],
          ...
          lentSectionTypeID: parseInt(FormDetails.lentSectionTypeID),
          ...
          lentdivisionCode: parseInt(FormDetails.lentdivisionID) == 0 ? '127' : '',
        ...
        <Container maxWidth={false}>
          <Formik
            initialValues={{
              ...
              lentSectionTypeID: FormDetails.lentSectionTypeID,
            ...
            validationSchema={
              Yup.object().shape({
                ...
                sectionTypeID: Yup.number().when([], {
                  is: () => !radioValue1,
                  then: Yup.number().required('Section type is required').min(1, 'Section type is required'),
                  otherwise: Yup.number().notRequired(),
                }),
                lentSectionTypeID: Yup.number().when([], {
                  is: () => radioValue1 && FormDetails.lentSectionTypeID == 0 && FormDetails.lentdivisionID != 0,
                  then: Yup.number().required('Lent section type is required').min(1, 'Lent section type is required'),
                  otherwise: Yup.number().notRequired(),
                }),
                ...
                lankemMusterChitNumber: Yup.string().required('Muster Chit Number is required').min(1, 'Muster ChitNumber is required').matches(/^\S*[^(. !@#$%&+)]\S*$/, 'Cannot contain only symbols and empty spaces'),
                lentEstateID: Yup.number().when([], {
                  is: () => radioValue1 && (FormDetails.lentEstateID == 0 && FormDetails.lentdivisionID == 0),
                ...
                lentdivisionID: Yup.number().when([], {
                  is: () => radioValue1 && (FormDetails.lentdivisionID == 0 && FormDetails.lentEstateID == 0),
                ...
                lentfieldID: Yup.number().when([], {
                  is: () => radioValue1 && FormDetails.lentfieldID == 0 && FormDetails.lentdivisionID != 0 && FormDetails.lentEstateID == 0,
                    ...
                    <PerfectScrollbar>
                      ...
                      <CardContent>
                        <Grid container spacing={2}>
                          ...
                          <Grid item md={3} xs={3}>
                            ...
                            <TextField select
                              ...
                              variant="outlined"
                              id="sectionTypeID"
                              disabled={FormDetails.lentdivisionID !== 0 || radioValue1}
                            >
                          ...
                          <Grid item md={3} xs={3}>
                            ...
                            <TextField
                              ...
                              onKeyDown={(e) => handleKeyDown(e, employeeCountRef)}
                            >
                            </TextField>
                          </Grid>
                          <Grid item md={3} xs={3}>
                            <InputLabel shrink id="collectedDate" style={{ marginBottom: '-8px' }}>
                              Date *
                            </InputLabel>
                        ...
                        <Grid container spacing={2}>
                          ...
                          <Grid item md={3} xs={3} hidden={FormDetails.lentdivisionID === 0}>
                            <InputLabel shrink id="lentSectionTypeID">
                              Lent Section Type *
                            </InputLabel>
                            <TextField select
                              error={Boolean(touched.lentSectionTypeID && errors.lentSectionTypeID)}
                              fullWidth
                              helperText={touched.lentSectionTypeID && errors.lentSectionTypeID}
                              name="lentSectionTypeID"
                              size='small'
                              onBlur={handleBlur}
                              onChange={(e) => {
                                handleChange(e);
                              }}
                              value={FormDetails.lentSectionTypeID}
                              variant="outlined"
                              id="lentSectionTypeID"
                              disabled={!radioValue1 || FormDetails.lentEstateID !== 0}
                            >
                              <MenuItem value={0}>--Select Lent Section Type--</MenuItem>
                              {generateDropDownMenu(lentsectionTypes)}
                            </TextField>
                          </Grid>
                          <Grid item md={3} xs={3} hidden={FormDetails.lentdivisionID === 0}>
                                ...
                                <Table className={classes.table} aria-label="caption table">
                                  <TableHead>
                                    <TableRow>
                                      ...
                                      <TableCell align="center" style={{ fontWeight: "bold" }}>Lent Section Type</TableCell>
                                  ...
                                  <TableBody>
                                    {musterChitDetails.map((rowData, index) => (
                                      <TableRow key={index}>
                                        ...
                                        <TableCell align='center' component="th" scope="row" style={{ borderBottom: "none" }}>
                                          {rowData.lentSectionTypeName !== undefined ? rowData.lentSectionTypeName : '-'}
                                        </TableCell>
                                        <TableCell align='center' component="th" scope="row" style={{ borderBottom: "none" }}>
