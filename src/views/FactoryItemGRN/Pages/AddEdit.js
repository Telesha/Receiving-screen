...
export default function FactoryGRNAddEdit() {
  ...
  async function DeleteItem(index) {
    ...
    handleInvoiceDateChange(new Date());
    handleReceiveDateChange(new Date());
    handleDueDateChange(new Date());
  ...
  async function saveFactoryGrn(values) {
    ...
    if (isUpdate == true) {
      ...
      if (factoryGRN.grnType == 2) {
        let countZeroSellingPrice = 0;
        let invalidPrices = false;

        tableItems.forEach(item => {
          if (item.unitPrice < item.buyingPrice && item.unitPrice !== 0) {
            invalidPrices = true;
          }
          if (item.unitPrice === 0) {
            countZeroSellingPrice++;
          }
        });

        if (invalidPrices) {
          alert.error("Selling price cannot be less than buying price.");
        } else if (countZeroSellingPrice > 0) {
          alert.error("Please enter the selling price.");
        }

        else {
          if (factoryGRN.poNumber != -1) {
            let response = await services.saveFactoryGrn(values, tableItems, dueDate, invoiceDate, receiveDate, factoryGRN, tempSuplierID);
            if (response.statusCode == "Success") {
              alert.success(response.message);
              setIsDisableButton(true);
              navigate('/app/factoryGRN/listing');
                          ...
                          <Formik
                            ...
                            validationSchema={
                              Yup.object().shape({
                                ...
                                unitPrice: Yup.string().matches(/^[0-9]*(\.[0-9]{0,2})?$/, 'Only allow numbers with at most two decimal places').test({
                                  name: 'unitPrice',
                                  message: 'Selling Price should be greater than or equal to Buying Price',
                                  test: function (value) {
                                    const buyingPrice = this.resolve(Yup.ref('buyingPrice'));
                                    return value >= buyingPrice;
                                  }
                                }).test('min', 'Selling Price can’t be zero', val => val > 0).nullable(),
