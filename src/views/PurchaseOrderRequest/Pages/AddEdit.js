...
export default function PurchaseOrderRequestAddEdit(props) {
    const [title, setTitle] = useState(" Add Purchase Order Request")
    ...
    const handleClick = () => {
        navigate('/app/purchaseOrderRequest/listing');
    }
    ...
    useEffect(() => {
        trackPromise(
            getLocationForDropDown(PurchaseDetails.groupID)
        )
    }, [PurchaseDetails.groupID])

    async function getPermission() {
        var permissions = await authService.getPermissionsByScreen(screenCode);
        var isAuthorized = permissions.find(p => p.permissionCode == 'ADDEDITPURCHASEORDERREQUEST');
    ...
    async function getLocationForDropDown(groupID) {
        var response = await services.getLocationForDropDown(groupID);
    ...
    async function getPurchaseOrderDetails(purchaseOrderRequestID) {
        let response = await services.getPurchaseOrderRequestDetails(purchaseOrderRequestID);
        let data = response.purchaseGeneral[0];
        let data1 = response.purchaseItemsList;
        setTitle("View Purchase Order Request");
    ...
    async function savePurchaseOrder(values) {
        ...
        let response = await services.savePurchaseOrderRequest(values, factoryDetails, supplierDetails, tableItems, totalNet, dueDate, poNumber);
                                        ...
                                        <CardContent>
                                            <Grid container spacing={3}>
                                                ...
                                                <Grid item md={3} xs={12}>
                                                    ...
                                                    >
                                                        <MenuItem value="0">--Select Factory--</MenuItem>
                                                        {generateDropDownMenu(locationList)}
                                                    </TextField>
                                                </Grid>
                                            </Grid>
