...
const screenCode = 'PURCHASEORDERREQUEST';

export default function PurchaseOrderRequestListing(props) {
    ...
    const handleClick = () => {
        encrypted = btoa('0');
        navigate('/app/purchaseOrderRequest/addedit/' + encrypted)
    }
    const handleClickEdit = (purchaseOrderRequestID) => {
        encrypted = btoa(purchaseOrderRequestID.toString());
        navigate('/app/purchaseOrderRequest/view/' + encrypted);
    }
    ...
    async function getPermission() {
        ...
        var isAuthorized = permissions.find(p => p.permissionCode == 'VIEWPURCHASEORDERREQUEST');
    ...
    async function getAllPurchaseOrderRequest() {
        var result = await services.getAllPurchaseOrderRequests(orderList.groupID, orderList.factoryID);
    ...
    async function getPurchaseOrderRequestByGroupIDFactoryID() {
        const response = await services.getPurchaseOrderRequestByGroupIDFactoryID(orderList.factoryID, orderList.supplierID);
            ...
            >
                ...
                <Container maxWidth={false}>
                    <Formik
                        ...
                        onSubmit={getPurchaseOrderRequestByGroupIDFactoryID}
                        enableReinitialize
                    >
                        {({
                            errors,
                            ...
                            <form onSubmit={handleSubmit}>
                                <Box mt={0}>
                                    <Card>
                                        <CardHeader
                                            title={cardTitle("Purchase Order Request")}
                                        />
                                        <PerfectScrollbar>
                                            <Divider />
                                                        ...
                                                        actions={[
                                                            {
                                                                ...
                                                                onClick: (event, factoryAdjustmentData) => handleClickEdit(factoryAdjustmentData.purchaseOrderRequestID)
                                                            },
                                                        ]}
                                                    /> : null}
                                            </Box>
