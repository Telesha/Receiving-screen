...
export default {
    ...
    getPurchaseOrderRequestByGroupIDFactoryID,
    getPurchaseOrderRequestDetails,
    getAllPurchaseOrderRequests,
...
async function getLocationForDropDown(groupID) {
    var factoryArray = [];
...
async function savePurchaseOrderRequest(PurchaseDetails, factoryDetails, supplierDetails, orderList, totalNet, dueDate, poNumber) {
    let model = {
        purchaseOrderRequestID: 0,
        groupID: parseInt(PurchaseDetails.groupID),
        factoryID: parseInt(PurchaseDetails.factoryID),
        locationID: parseInt(PurchaseDetails.locationID),
    ...
    const response = await CommonPost('/api/PurchaseOrder/SavePurchaseOrderRequests', null, model);
    return response;
}

async function getPurchaseOrderRequestByGroupIDFactoryID(factoryID, supplierID) {
    const response = await CommonGet('/api/PurchaseOrder/GetPurchaseOrderRequestListing', "factoryID=" + parseInt(factoryID) + '&supplierID=' + parseInt(supplierID));
    return response.data;
}

async function getPurchaseOrderRequestDetails(purchaseOrderRequestID) {
    const response = await CommonGet('/api/PurchaseOrder/GetPurchaseOrderRequestDetails', "purchaseOrderRequestID=" + parseInt(purchaseOrderRequestID));
    return response.data;
}

async function getAllPurchaseOrderRequests(groupID, factoryID) {
    const response = await CommonGet('/api/PurchaseOrder/GetAllPurchaseOrderRequests', "groupID=" + parseInt(groupID) + "&factoryID=" + parseInt(factoryID));
    return response;
