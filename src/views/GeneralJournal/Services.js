...
async function getReferenceNumber(model) {
  const response = await CommonGet('/api/AuditTrialReport/GetModifiedVoucherCodeForJournals', 'groupID=' + model.groupID + '&factoryID='
...
async function saveGeneralJournal(generalJournal) {
  let saveModel = {
    ...
    referenceNumberInter: (generalJournal.referenceNumberInter.toString()),
