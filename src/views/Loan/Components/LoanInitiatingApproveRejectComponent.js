...
export const LoanInitiating = ({
    ...
    const minMonthDate = moment(MinMonth, 'YYYY-MM');
    const effectiveMonthDate = moment(FullLoanDetails.paymentEffectiveMonth, 'YYYY-MM');
    const [PaymentEfectedMonth, setPaymentEfectedMonth] = useState(null);
    ...
    useEffect(() => {
        let object = {
            ...
            paymentMonth: FullLoanDetails.paymentEffectiveMonth
        }
    ...
    useEffect(() => {
        if (FullLoanDetails !== null) {
            ...
            setPaymentEfectedMonth(FullLoanDetails.paymentEffectiveMonth)
        }
    }, []);

    useEffect(() => {
        if (FullLoanDetails) {
            setPaymentEfectedMonth(effectiveMonthDate.format('YYYY-MM'));
            if (effectiveMonthDate.isSameOrBefore(minMonthDate) && !IsViewOnly) {
                setIsEditable(true);
            } else {
                setIsEditable(false);
            }
        }
    }, [FullLoanDetails, MinMonth]);
    ...
    function EqualMonthlyInstalmentLoanCalculator(obj) {
        ...
        let date = new Date(obj.paymentMonth);
    ...
    async function ApproveLoanRequest() {
        ...
        if (moment(PaymentEfectedMonth).isSame(moment(MinMonth), 'month') && moment(PaymentEfectedMonth).isSame(moment(MinMonth), 'year')) {
            alert.error('Please change the Payment Effective Month');
        ...
        let approveObject = {
            ...
            paymentEffectiveMonth: moment(PaymentEfectedMonth).format('YYYY-MM-DD'),
                                    ...
                                    <Grid item md={6} xs={12}>
                                        ...
                                        <MuiPickersUtilsProvider utils={DateFnsUtils}>
                                            <KeyboardDatePicker
                                                ...
                                                fullWidth
                                                minDate={moment(MinMonth).add(1, 'month').startOf('month').toDate()}
                                                helperText={touched.paymetEffectedMonth && errors.paymetEffectedMonth}
                                                inputVariant="outlined"
                                                openTo="month"
                                                views={["year", "month"]}
                                                id="paymetEffectedMonth"
                                                value={PaymentEfectedMonth}
                                                onChange={(date) => {
                                                    setPaymentEfectedMonth(date);
                                                }}
                                                disabled={!isEditable}
                                                KeyboardButtonProps={{
